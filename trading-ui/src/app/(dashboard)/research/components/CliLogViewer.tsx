        ...
        {isBrowserActive && (
          <div className="absolute inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-300">
            <div className={`flex flex-wrap items-center justify-center gap-6 w-full max-w-[1400px] max-h-[90vh] overflow-y-auto p-4 custom-scrollbar`}>
              {browsersList.map(([id, b]) => (
                <div key={id} className={`flex flex-col relative rounded-2xl border border-emerald-500/40 bg-emerald-950/20 backdrop-blur-xl overflow-hidden p-3 sm:p-4 shadow-[0_0_40px_rgba(16,185,129,0.3)] animate-in zoom-in-95 duration-500 ${browsersList.length > 1 ? 'w-full lg:w-[46%] max-w-[650px]' : 'w-[90%] max-w-[900px]'}`}>
                    ...
                    <div className="flex-1 p-0 overflow-hidden flex flex-col relative bg-white group">
                      {b.url ? (
                        <>
                          ...
                          <img 
                            key={b.url}
                            src={`https://api.microlink.io/?url=${encodeURIComponent(b.url)}&screenshot=true&meta=false&embed=screenshot.url`}
